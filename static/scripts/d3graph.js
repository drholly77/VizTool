...
function setClick(obj, d) {
  obj.onclick = () => {
    ...
    if (d.name == "Start") {
      ...
      document.querySelector("#avg-distance").innerHTML = d.distance;  // added
      document.querySelector("#straight2correct").innerHTML = `${Math.round(d.score * 100)}% (${Math.round(d.score * d.appearances)})`;
...
function addnewNodes(nodeDataArray, reInitMergeList) {
  ...
  newNodes.append("text")
    ...
    .html(d => ((d.name == 'Start' || d.name == 'Gave Up') ? d.name : (d.name.match(/T/g) || []).length)) // here
...
// added
var testCasesAndAppearances = []
graph.data.nodes.forEach((n) => {
  if (n.name === "Gave Up") {
    document.querySelector("#gave-up").innerHTML = n.appearances
  }
  testCasesAndAppearances.push([n.appearances, n.name, new Set(n.users).size])
})
testCasesAndAppearances.sort(function(a, b) {
  var y = a[0]; var x = b[0];
  return ((x < y) ? -1 : ((x > y) ? 1 : 0));
})

testCasesAndAppearances.forEach((c) => {
  const testCase = document.createElement("li")
  const node = document.createTextNode(c[1] + " (" + c[0] + "/" + c[2] + ")")
  testCase.appendChild(node)
  document.querySelector("#failures").appendChild(testCase)
})
