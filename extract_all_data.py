...
def massage(file):
    ...
    # outer_folder_name = file[:-4]
    # #print("original outer", outer_folder_name)
    # with ZipFile(UPLOAD_DIR, 'r') as zipfile:
    #     for folder in zipfile.infolist():
    #         if folder.is_dir() and not (folder.filename.startswith('__') or 'MACOSX' in folder.filename or 'MACOS' in folder.filename):
    #             outer_folder_name = folder.filename.split('-')[0]

    outer_folder_name = Path(file).stem   # setting the assignmnet name to the name of the zip folder
    # 1. not to add to the original warmup file if new data was added but to split it
    # 2. not to cause an error
    # doing it here bc here we put data in the outer folder that extract_dtuff later uses for naming csv files

    #print("after outer", outer_folder_name)
    ...
    for assignment in os.listdir(DATA_DIR):
        for folder in os.listdir(f'{DATA_DIR}/{assignment}'):
            ...
            for val in np.unique(df[0]):
                ...
                val = f'{assignment}_{val.replace("_","-")}'
