...
PYTHONANYWHERE_DIR = ''
DATA_BASE_FOLDER = PYTHONANYWHERE_DIR + 'PARSONS_DATA/'


def clean_up_data_directory():
    shutil.rmtree(DATA_BASE_FOLDER)
    ...
    # Path(PYTHONANYWHERE_DIR + 'PARSONS_DATA/clean_after_extract_all').mkdir(parents=True, exist_ok=True)


def extract_stuff(assignment_name, uploads_folder, override=True):
    # datafolder = 'clean_after_original_massage'   # changed from 'data'
    
    FILE_DIR = PYTHONANYWHERE_DIR + f'{uploads_folder}/{assignment_name}'

    assignment_name_without_extension = '.'.join(assignment_name.split('.')[:-1])
    datafolder = FILE_DIR
    cleanfolder = PYTHONANYWHERE_DIR + 'PARSONS_DATA/clean_after_extract_stuff'
    ...
    with open(PYTHONANYWHERE_DIR + 'parson_db.txt', 'a') as file, open(FILE_DIR, 'r') as f:
        file.write(assignment_name_without_extension + "|" + ''.join(f.readlines()).replace('\n', SPLIT) + '\n')
